...
export function registerTools(api: any) {
    ...
    api.registerTool({
        ...
        async execute(_toolCallId: string, args: Record<string, unknown>) {
            ...
            if (!apiKey) return { content: [{ type: "text", text: `Error: ${checkConfigText}` }] };

            const prompt = args.prompt as string;
            const size = (args.size as string) ?? "1024x1024";
            const n = (args.count as number) ?? 1;
            ...
            try {
                ...
                const resultText = images.map((img: any, i: number) => `Image ${i + 1}: ${img.url}`).join("\n");
                return {
                    content: [
